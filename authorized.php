...
if (isset($_GET['oauth_token']) || (isset($_COOKIE['oauth_token']) && isset($_COOKIE['oauth_token_secret']))) {
    ...
    if (isset($params['user_id'])) {
        ...
        if (count($r) == 0) { //store the user info as it doesn't exist already
            ...
            $_SESSION['reg'] = 1;
            $sql = "UPDATE auth SET user_name='" . $params['screen_name'] . "', oauth_token='" . $params['oauth_token'] . "', oauth_token_secret='" . $params['oauth_token_secret'] . "' WHERE user_id=" . $_SESSION['user_id'];
            $myDB->query($sql);
